...
# Modified web version of prepareData which directly accepts file and always uses default policy for OOV. 
def prepare_web_data(file, delimiter = '\t'):
...
def evaluate_web_data(file, delimiter = '\t'):
    truncations = []
    replacements = {}

    message = "Evaluated data. Results: \n"
    ...
    if len(oov) > 0:
        for word in set(oov):
                ...
                if len(closest_word)>0 and nltk.edit_distance(word, closest_word[0]) <= 2:
                    ...
                    while word in df.values:
                        trunc(word, df)
                        truncations.append(word)
                df.replace(replacements, inplace=True)    
    return len(oov), replacements, truncations