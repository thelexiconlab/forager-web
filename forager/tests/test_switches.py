...
freq_matrix = pd.read_csv(parentfolder + "lexical_data/USE_frequencies.csv", header=None)
labels = list(freq_matrix[0])
freq_matrix = np.array(freq_matrix[1])
sim_matrix = pd.read_csv(parentfolder + 'lexical_data/USE_semantic_matrix.csv', delimiter=',', header=None).values
phon_matrix = pd.read_csv(parentfolder + 'lexical_data/USE_phonological_matrix.csv', header=None).values
...
def test_delta():
    ...
    assert switch_delta(ex_list_1,history_vars[0],1,1) == [2, 0, 0, 0, 1, 0] # USE_semantic_matrix has different similarity values
